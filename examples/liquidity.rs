...
use bittensor_rs::queries::{liquidity as lq, subnets};
    ...
    match lq::get_liquidity_list(&client, netuid, &coldkey, None).await {
        Ok(positions) => {
            ...
            if !positions.is_empty() {
                let current_price_rao = lq::get_current_subnet_price_rao(&client, netuid)
                    .await
                    .unwrap_or(0);
                let (alpha_rao, tao_rao) = positions[0].to_token_amounts(current_price_rao);
                println!(
                    "pos0 id={} alpha_rao={} tao_rao={} fees_tao_rao={} fees_alpha_rao={}",
                    positions[0].id,
                    alpha_rao,
                    tao_rao,
                    positions[0].fees_tao_rao,
                    positions[0].fees_alpha_rao
                );
            }
        }
