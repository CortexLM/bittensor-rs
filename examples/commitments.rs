...
use bittensor_rs::queries::{subnets, commitments};
...
async fn main() -> Result<()> {
	...
	let revealed = commitments::get_revealed_commitment_by_hotkey(&client, netuid, &hotkey).await.unwrap_or_default();
	let last_reset = commitments::get_last_commitment_bonds_reset_block(&client, netuid, &hotkey).await.ok().flatten();
	let wci_v2 = commitments::get_current_weight_commit_info_v2(&client, netuid).await.unwrap_or_default();
	println!("revealed_count={} last_reset_block={:?} weight_commit_info_v2_items={} ", revealed.len(), last_reset, wci_v2.len());
	Ok(())
}
