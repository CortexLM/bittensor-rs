...
pub async fn add_stake_multiple(
    ...
    if hotkeys.len() != amounts.len() {
        return Err(anyhow::anyhow!("Hotkeys and amounts must have the same length"));
    }
    ...
    client
        .submit_extrinsic(SUBTENSOR_MODULE, "add_stake_multiple", args, signer, wait_for)
        .await
...
pub async fn unstake_multiple(
    ...
    client
        .submit_extrinsic(SUBTENSOR_MODULE, "remove_stake_multiple", args, signer, wait_for)
        .await
