...
async fn main() -> Result<()> {
	...
	let base = subxt::storage::dynamic("SubtensorModule", "Keys", vec![Value::u128(netuid as u128)]);
	let mut iter = storage.at_latest().await?.iter(base).await?;
	let mut uid_to_hotkey: Vec<(u64, sp_core::crypto::AccountId32)> = Vec::with_capacity(n as usize);
	...
	let results: Vec<_> = stream::iter(uid_to_hotkey.into_iter())
		.map(|(uid, hotkey)| {
			...
			async move {
				let hot_bytes: &[u8] = <sp_core::crypto::AccountId32 as AsRef<[u8]>>::as_ref(&hotkey);
				
				// Get owner (coldkey)
				let owner = client
					.storage_with_keys("SubtensorModule", "Owner", vec![Value::from_bytes(hot_bytes)])
					.await
					.ok()
					.flatten()
	...
	println!("\n{:<5} {:<56} {:<56} {:>14} {:>14} {:>10} {:>10} {:>10} {:>10} {:>10}", 
		"UID", "Hotkey", "Coldkey", "Stake(RAO)", "Emission", "Incentive", "Trust", "Consensus", "VTrust", "Dividends");
	println!("{}", "─".repeat(200));
	
	for (uid, hot, cold, stake, emis, incentive, trust, consensus, vtrust, dividends) in results.iter().take(15) {
		let hot_ss58 = hot.to_ss58check_with_version(sp_core::crypto::Ss58AddressFormat::custom(SS58_FORMAT));
		let cold_ss58 = cold.as_ref()
			.map(|c| c.to_ss58check_with_version(sp_core::crypto::Ss58AddressFormat::custom(SS58_FORMAT)))
		...
		println!("{:<5} {:<56} {:<56} {:>14} {:>14} {:>10.6} {:>10.6} {:>10.6} {:>10.6} {:>10.6}",
			uid, hot_ss58, cold_ss58, stake, emis, incentive_f, trust_f, consensus_f, vtrust_f, dividends_f);
	}
	
	Ok(())
}

async fn fetch_normalized_vec(client: &BittensorClient, storage_name: &str, netuid: u16) -> Result<Vec<u16>> {
	client
		.storage_with_keys("SubtensorModule", storage_name, vec![Value::u128(netuid as u128)])
		.await?
