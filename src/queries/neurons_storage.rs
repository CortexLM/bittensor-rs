...
use crate::types::{NeuronInfo, AxonInfo, PrometheusInfo};
...
pub async fn query_neuron_from_storage(
    ...
    let coldkey_val = client
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("Coldkey (Owner) not found for hotkey on subnet {}", netuid))?;
    ...
    let version = fetch_u64_storage_opt(client, SUBTENSOR_MODULE, "Version", uid_key.clone()).await.unwrap_or(None)
        .unwrap_or(0);
    ...
    {
        Ok(Some(stake_val)) => {
            decode_vec_account_u128_pairs(&stake_val)
                .unwrap_or_else(|_| Vec::new())
        }
        _ => Vec::new()
    };
    let mut stake_dict: HashMap<AccountId32, u128> = HashMap::new();
    for (ck, amt) in stake_entries { stake_dict.insert(ck, amt); }
    ...
    let prometheus_info = fetch_prometheus_info(client, SUBTENSOR_MODULE, "Prometheus", axon_keys).await;
...
async fn fetch_normalized_u16_storage_opt(
    ...
    if let Some(value) = client.storage_with_keys(module, entry, keys).await? {
        Ok(Some(decode_normalized_u16(&value).context("Failed to decode normalized u16")?))
    } else {
        Ok(None)
    }
}
