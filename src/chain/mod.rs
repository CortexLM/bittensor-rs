...
pub use runtime::*;
pub use signer::{BittensorSigner, create_signer, signer_from_seed};
...
impl BittensorClient {
        ...
        match value {
            Some(thunk) => {
                ...
                match thunk.to_value() {
                    ...
                    Err(e) => Err(Error::Decoding(format!("Failed to decode storage value: {}", e))),
                }
            }
            None => Ok(None),
        }
    ...
    pub async fn runtime_api(
        ...
        match result.to_value() {
            Ok(v) => Ok(Some(v.remove_context())),
            Err(e) => Err(Error::Decoding(format!("Failed to decode runtime API result: {}", e))),
        }
    }
            ...
            Some(thunk) => {
                ...
                if let Ok(named) = crate::utils::scale_decode::decode_named_composite(&value) {
                    ...
                    if let Some(data_value) = named.get("data") {
                        if let Ok(data_fields) = crate::utils::scale_decode::decode_named_composite(data_value) {
                            // Extract 'free' balance
                            if let Some(free_value) = data_fields.get("free") {
                                if let Ok(balance) = crate::utils::scale_decode::decode_u128(free_value) {
                                    return Ok(balance);
                                }
                            }
                        }
                ...
                Err(Error::Decoding("Failed to extract balance from account data".to_string()))
            }
            None => Err(Error::Decoding("Account balance not found in storage".to_string())),
        }
    }
    ...
    pub async fn submit_extrinsic(
        ...
        let mut tx_client = self.api.tx().sign_and_submit_then_watch_default(&call, signer).await?;
        ...
        match wait_for {
                ...
                let in_block = loop {
                    match tx_client.next().await {
                        Some(Ok(status)) => match status {
                            ...
                            subxt::tx::TxStatus::Error { message } => return Err(Error::Transaction(format!("Transaction error: {}", message))),
                            subxt::tx::TxStatus::Invalid { message } => return Err(Error::Transaction(format!("Invalid transaction: {}", message))),
                            subxt::tx::TxStatus::Dropped { message } => return Err(Error::Transaction(format!("Transaction dropped: {}", message))),
                            _ => continue,
                        },
                        Some(Err(e)) => return Err(Error::Transaction(format!("Transaction status error: {}", e))),
                        None => return Err(Error::Transaction("Transaction stream ended unexpectedly".to_string())),
                    }
                };
            ...
            ExtrinsicWait::None => {
                Ok(format!("{:?}", tx_client.extrinsic_hash()))
            }
        }
    }
    ...
    pub async fn block_number(&self) -> Result<u64, Error> {
        ...
        let header = self.api.backend().block_header(finalized_head.hash()).await
    ...
    pub async fn block_hash(&self, block_number: u64) -> Result<Option<sp_core::H256>, Error> {
        ...
        match backend.block_header(backend.latest_finalized_block_ref().await?.hash()).await {
    ...
    pub async fn query_constant(
        ...
        Err(Error::Decoding(format!("Constant {}.{} not found in metadata", module, constant)))
    }
}
