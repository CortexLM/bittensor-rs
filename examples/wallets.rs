...
async fn main() -> Result<()> {
	...
	let coldkey = coldkey_val.and_then(|v| bittensor_rs::utils::value_decode::decode_account_id32(&v).ok());

	let exist = wallets::does_hotkey_exist(&client, &hotkey).await.unwrap_or(false);
	let reg_any = wallets::is_hotkey_registered_any(&client, &hotkey).await.unwrap_or(false);
	let owned = match &coldkey { Some(c) => wallets::get_owned_hotkeys(&client, c).await.unwrap_or_default(), None => Vec::new() };
	println!("exists={} registered_any={} owned_hotkeys_count={}", exist, reg_any, owned.len());
	Ok(())
}
