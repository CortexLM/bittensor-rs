...
pub async fn get_balance(
    ...
    client.account_balance(account).await.map_err(|e| anyhow::anyhow!("{}", e))
}
...
pub async fn get_balances(
    ...
    let _keys: Vec<_> = accounts
        .iter()
        .map(|acc| subxt::dynamic::storage("System", "Account", vec![Value::from_bytes(&acc.encode())]))
        .collect();

    // No fetch_many in current subxt version; perform sequential fetches
    let mut out = Vec::with_capacity(accounts.len());
    for acc in accounts.iter() {
        let addr = subxt::dynamic::storage("System", "Account", vec![Value::from_bytes(&acc.encode())]);
        let res = storage.fetch(&addr).await?;
        if let Some(thunk) = res {
            let value = thunk.to_value().map_err(|e| anyhow::anyhow!("decode: {}", e))?.remove_context();
            let s = format!("{:?}", value);
            let mut free: u128 = 0;
            if let Some(pos) = s.find("free") {
                ...
                for ch in trimmed.chars() { if ch.is_ascii_digit() { num.push(ch); } else { break; } }
                if !num.is_empty() { free = num.parse::<u128>().unwrap_or(0); }
            }
            out.push(free);
        } else {
            out.push(0);
...
pub async fn get_existential_deposit(client: &BittensorClient) -> Result<u128> {
    ...
    let value = client.query_constant("Balances", "ExistentialDeposit").await?
