...
async fn main() -> Result<()> {
	...
	let owner = subnets::subnet_owner_hotkey(&client, netuid).await.ok().flatten();
	println!("owner_hotkey={:?}", owner.as_ref().map(|a| hex::encode(<sp_core::crypto::AccountId32 as AsRef<[u8]>>::as_ref(a))));

	let price_rao = subnets::get_subnet_price(&client, netuid).await.unwrap_or(0);
