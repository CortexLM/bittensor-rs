...
fn test_normalize_max_weight_uneven_weights() {
    ...
    assert!(max_w <= limit + 0.01, "Max weight {} exceeds limit {}", max_w, limit);
...
fn test_normalize_weights_basic() {
    ...
    assert!((mid / side - 2.0).abs() < 0.1, "Middle weight should be ~2x side weights");
}
...
fn test_normalize_weights_all_zeros() {
    ...
    for w in &result_weights {
        assert!((*w as i32 - first as i32).abs() < 100, "Weights should be approximately equal");
    }
}
