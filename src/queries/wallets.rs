...
pub async fn does_hotkey_exist(client: &BittensorClient, hotkey: &AccountId32) -> Result<bool> {
        ...
        .await?
    {
        ...
        if let Some(pos) = s.find("0x") {
            let hex = &s[pos+2..].chars().take_while(|c| c.is_ascii_hexdigit()).collect::<String>();
            if hex.len() >= 64 {
...
pub async fn is_hotkey_registered_any(client: &BittensorClient, hotkey: &AccountId32) -> Result<bool> {
...
pub async fn get_owned_hotkeys(client: &BittensorClient, coldkey: &AccountId32) -> Result<Vec<AccountId32>> {
    if let Some(val) = client
        .storage_with_keys(SUBTENSOR_MODULE, "OwnedHotkeys", vec![Value::from_bytes(&coldkey.encode())])
        .await?
    {
...
pub async fn get_hotkey_owner(client: &BittensorClient, hotkey: &AccountId32) -> Result<Option<AccountId32>> {
    if let Some(val) = client
        .storage_with_keys(SUBTENSOR_MODULE, "Owner", vec![Value::from_bytes(&hotkey.encode())])
        .await?
    {
