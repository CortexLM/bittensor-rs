...
use crate::core::constants::{MIN_TICK, MAX_TICK, TICK_STEP};
use crate::types::LiquidityPosition;
use crate::utils::scale_decode::{decode_u64, decode_u128, decode_i32, decode_fixed_u64f64, decode_named_composite, decode_vec};
use anyhow::Result;
...
async fn fetch_positions_value(
	...
	if let Ok(val) = client
		.storage_with_keys(SWAP_PALLET, "Positions", vec![Value::from_bytes(&coldkey.encode())])
		.await
	{
		if val.is_some() { return Ok(val); }
	}
	...
	if let Ok(val) = client
		.storage_with_keys(
			SWAP_PALLET,
			"Positions",
			vec![Value::u128(netuid as u128), Value::from_bytes(&coldkey.encode())],
		)
		.await
	{
		if val.is_some() { return Ok(val); }
	...
	if let Ok(val) = client
		.storage_with_keys(
			SWAP_PALLET,
			"Positions",
			vec![Value::from_bytes(&coldkey.encode()), Value::u128(netuid as u128)],
		)
		.await
	{
		if val.is_some() { return Ok(val); }
...
pub async fn get_liquidity_list(
	...
	let fee_global_tao = read_fixed_u64f64(client, SWAP_PALLET, "FeeGlobalTao", vec![Value::u128(netuid as u128)]).await?;
	let fee_global_alpha = read_fixed_u64f64(client, SWAP_PALLET, "FeeGlobalAlpha", vec![Value::u128(netuid as u128)]).await?;
	let sqrt_price = read_fixed_u64f64(client, SWAP_PALLET, "AlphaSqrtPrice", vec![Value::u128(netuid as u128)]).await?;
	...
	let Some(positions_val) = positions_val else { return Ok(Vec::new()); };
	...
	if let Ok(positions) = decode_vec(&positions_val, |v| Ok(v.clone())) {
		for position_val in positions {
			let (id, tick_low, tick_high, liquidity, pos_netuid) = parse_position_fields(&position_val);
			if let Some(n) = pos_netuid { if n as u16 != netuid { continue; } } else { continue; }
			...
			let tao_below = get_fees(current_tick, tick_low_map, tick_low, true, fee_global_tao, fee_global_alpha, false);
			let tao_above = get_fees(current_tick, tick_high_map, tick_high, true, fee_global_tao, fee_global_alpha, true);
			let alpha_below = get_fees(current_tick, tick_low_map, tick_low, false, fee_global_tao, fee_global_alpha, false);
			let alpha_above = get_fees(current_tick, tick_high_map, tick_high, false, fee_global_tao, fee_global_alpha, true);
			...
			out.push(LiquidityPosition{
				...
				fees_alpha_rao: if fees_alpha <= 0.0 { 0 } else { fees_alpha as u128 },
				netuid,
			});
		}
	}
...
pub async fn get_current_subnet_price_rao(client: &BittensorClient, netuid: u16) -> Result<u128> {
	...
	Ok(if price <= 0.0 { 0 } else { (price * 1e9f64) as u128 })
}
...
fn parse_position_fields(value: &Value) -> (Option<u64>, i32, i32, u128, Option<u64>) {
	...
	if let Ok(fields) = decode_named_composite(value) {
		...
		let tick_low = fields.get("tick_low").and_then(|v| decode_i32(v).ok()).unwrap_or(0);
		let tick_high = fields.get("tick_high").and_then(|v| decode_i32(v).ok()).unwrap_or(0);
		let liquidity = fields.get("liquidity").and_then(|v| decode_u128(v).ok()).unwrap_or(0);
...
async fn read_fixed_u64f64(client: &BittensorClient, pallet: &str, entry: &str, keys: Vec<Value>) -> Result<f64> {
	let val = client.storage_with_keys(pallet, entry, keys).await?
...
async fn fetch_tick_value(client: &BittensorClient, tick: i32, netuid: u16) -> Result<Option<Value>> {
	// 1) Single key: i128
	if let Ok(val) = client
...
fn get_fees(
	...
	let tick_fee_key = if quote { "fees_out_tao" } else { "fees_out_alpha" };
	let tick_fee_value = tick_val
		.and_then(|v| extract_fixed_field(v, tick_fee_key))
		.unwrap_or(0.0);
	let global_fee_value = if quote { global_fees_tao } else { global_fees_alpha };
	if above {
		if tick_index <= current_tick { global_fee_value - tick_fee_value } else { tick_fee_value }
	} else {
		if tick_index <= current_tick { tick_fee_value } else { global_fee_value - tick_fee_value }
	}
}
...
#[allow(dead_code)]
fn extract_first_u128(s: &str) -> Option<u128> { if let Some(pos) = s.find("U128(") { let aft = &s[pos+5..]; if let Some(end)=aft.find(')'){ return aft[..end].trim().parse::<u128>().ok(); } } None }
fn extract_first_u128_after_key(s: &str, key: &str) -> Option<u128> { if let Some(kp) = s.find(key) { let subs = &s[kp..]; if let Some(p)=subs.find("U128(") { let aft = &subs[p+5..]; if let Some(end) = aft.find(')'){ return aft[..end].trim().parse::<u128>().ok(); } } } None }
fn extract_first_u64_after_key(s: &str, key: &str) -> Option<u64> {
	if let Some(kp) = s.find(key) {
		let subs = &s[kp..];
		if let Some(p) = subs.find("U64(") { let aft = &subs[p+4..]; if let Some(end) = aft.find(')') { return aft[..end].trim().parse::<u64>().ok(); } }
		if let Some(p) = subs.find("I64(") { let aft = &subs[p+4..]; if let Some(end) = aft.find(')') { return aft[..end].trim().parse::<i64>().ok().and_then(|v| if v>=0 { Some(v as u64) } else { None }); } }
	}
	None
}
fn extract_first_i32_after_key(s: &str, key: &str) -> Option<i32> {
	if let Some(kp) = s.find(key) {
		let subs = &s[kp..];
		if let Some(p) = subs.find("I32(") { let aft = &subs[p+4..]; if let Some(end) = aft.find(')') { return aft[..end].trim().parse::<i32>().ok(); } }
		if let Some(p) = subs.find("U32(") { let aft = &subs[p+4..]; if let Some(end) = aft.find(')') { return aft[..end].trim().parse::<u32>().ok().map(|v| v as i32); } }
	}
	None
}
...
fn price_to_tick(price: f64) -> i32 {
	...
	if tick < MIN_TICK { MIN_TICK } else if tick > MAX_TICK { MAX_TICK } else { tick }
}

fn tick_to_price(tick: i32) -> f64 {
	if tick < MIN_TICK || tick > MAX_TICK { return 0.0; }
	TICK_STEP.powi(tick)
}
