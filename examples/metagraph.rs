...
async fn main() -> Result<()> {
	...
	for (i, neuron) in neurons_list.iter().take(10).enumerate() {
		let hot_ss58 = neuron.hotkey.to_ss58check_with_version(
			sp_core::crypto::Ss58AddressFormat::custom(SS58_FORMAT)
		);
		let cold_ss58 = neuron.coldkey.to_ss58check_with_version(
			sp_core::crypto::Ss58AddressFormat::custom(SS58_FORMAT)
		);
		...
		if let Some(ref axon) = neuron.axon_info {
			if axon.is_serving() {
				println!("Axon:             {}:{} (v{})", axon.ip, axon.port, axon.version);
			} else {
		...
		if let Some(ref prom) = neuron.prometheus_info {
			if prom.is_serving() {
				println!("Prometheus:       {}:{} (v{})", prom.ip, prom.port, prom.version);
			}
		}
		
		// Display weights if not empty
		if !neuron.weights.is_empty() && neuron.weights.len() <= 5 {
			println!("Weights ({}):      {:?}", neuron.weights.len(), neuron.weights);
