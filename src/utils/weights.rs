...
pub fn normalize_max_weight(weights: &[f32], limit: f32) -> Vec<f32> {
    ...
    // Apply cutoff
    let clipped: Vec<f32> = weights.iter()
...
pub fn convert_weight_uids_and_vals_to_tensor(
    n: usize,
    uids: &[u16],
    weights: &[u16],
) -> Vec<f32> {
...
pub fn convert_bond_uids_and_vals_to_tensor(
    n: usize,
    uids: &[u16],
    bonds: &[u64],
) -> Vec<u64> {
...
pub fn process_weights(
    ...
    let non_zero: Vec<(u64, f32)> = uids.iter()
        .zip(weights.iter())
        .filter(|(_, w)| **w > 0.0)
        .map(|(u, w)| (*u, *w))
        .collect();
    ...
    // Filter by quantile
    let filtered: Vec<(u64, f32)> = non_zero.into_iter()
