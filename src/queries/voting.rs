...
pub async fn get_vote_data(client: &BittensorClient, proposal_hash: H256) -> Result<Option<ProposalVoteData>> {
	if let Some(val) = client
		.storage_with_keys(TRI_PALLET, "Voting", vec![Value::from_bytes(proposal_hash.as_bytes())])
		.await?
	{
		let s = format!("{:?}", val);
		...
		return Ok(Some(ProposalVoteData { index, threshold, ayes, nays, end }));
	}
	Ok(None)
}

fn extract_first_u64_after_key(s: &str, key: &str) -> Option<u64> { if let Some(kp)=s.find(key){ let subs=&s[kp..]; if let Some(p)=subs.find("U64("){ let aft=&subs[p+4..]; if let Some(end)=aft.find(')'){ return aft[..end].trim().parse::<u64>().ok(); } } if let Some(p)=subs.find("U32("){ let aft=&subs[p+4..]; if let Some(end)=aft.find(')'){ return aft[..end].trim().parse::<u32>().ok().map(|v| v as u64); } } } None }
...
fn extract_accounts_array_after_key(s: &str, key: &str) -> Vec<AccountId32> {
	...
	if let Some(kp) = s.find(key) {
		...
		while let Some(pos) = rem.find("0x") {
			let hexstr: String = rem[pos+2..].chars().take_while(|c| c.is_ascii_hexdigit()).collect();
			if hexstr.len() >= 64 {
