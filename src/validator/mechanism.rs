...
pub async fn commit_mechanism_weights(
    ...
    client
        .submit_extrinsic(SUBTENSOR_MODULE, "commit_mechanism_weights", args, signer, wait_for)
        .await
...
pub async fn reveal_mechanism_weights(
    ...
    let salt_values: Vec<Value> = salt.iter().map(|s| Value::u128((*s as u64) as u128)).collect();
    ...
    client
        .submit_extrinsic(SUBTENSOR_MODULE, "reveal_mechanism_weights", args, signer, wait_for)
        .await
...
pub async fn set_mechanism_weights(
    ...
    let version = version_key.ok_or_else(|| anyhow::anyhow!("Version key is required for commit_mechanism_weights"))?;
    ...
    client
        .submit_extrinsic(SUBTENSOR_MODULE, "set_mechanism_weights", args, signer, wait_for)
        .await
