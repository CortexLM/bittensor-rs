...
use bittensor_rs::queries::{delegates, subnets};
...
async fn main() -> Result<()> {
    ...
    let is_del = delegates::is_hotkey_delegate(&client, &hotkey)
        .await
        .unwrap_or(false);
    ...
    if is_del {
        match delegates::get_delegate_by_hotkey(&client, &hotkey).await {
            Ok(Some(info)) => println!(
                "delegate_info={{ hotkey={}, take={}, nominators={}, total_stake={:?} }}",
                info.base.hotkey_ss58.to_ss58check_with_version(
